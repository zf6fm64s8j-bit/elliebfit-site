...
status: active
verification: pass
next_action: "Paste the GA4 Measurement ID into assets/analytics.js, and click Start authentication in Google Admin now that DKIM has propagated. Deferred audit items: local-search pages, rates examples, privacy notice, staging environment."
---
...
10. **Analytics wired but off.** `assets/analytics.js` holds the only switch; `GA_MEASUREMENT_ID` is
   blank, so nothing loads until an ID is pasted in.
...
Two owner items, neither blocking: paste the GA4 Measurement ID into `assets/analytics.js`, and
click **Start authentication** in Google Admin now that the DKIM record has propagated. Then
