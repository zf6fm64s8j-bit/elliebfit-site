...
11. **Email authentication complete and confirmed in the wild.** The Namecheap DNS edit dropped
   every apex TXT record; MX survived, so inbound mail never broke, but outbound was
   unauthenticated. SPF, DKIM and DMARC are now all published: `v=spf1 include:_spf.google.com
   ~all` (1 of 10 DNS lookups used) and `google._domainkey` carrying a valid RSA-2048 key that
   reassembles byte-identically from its two DNS string chunks. Two messages Ellen forwarded on
   2026-08-21 carry `dkim=pass header.s=google`, `spf=pass`, `dmarc=pass` in their own headers —
   real-world proof, not just a well-formed record.
...
142.112.48.163** — a Bell Canada residential DSL static IP in Ontario, not in the SPF record —
with `dkim=fail`, `spf=softfail`, and `disposition=none`. Low-volume spoofing of a small domain is
...
**The trade accepted:** with `rua` gone there is no longer any visibility into what fails. The
feedback loop is now a person saying they did not receive Ellen's email. If that ever happens,
re-add `rua` and diagnose before assuming the policy is at fault.

**If reports are ever wanted back:** `rua` must point at an address **on elliebfit.com**. A Gmail
address will not work — RFC 7489 requires the report address's domain to publish an authorizing
